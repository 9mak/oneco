...
- **画像ホストの列挙**: 新しい収集サイトの画像ホストは `next.config.ts` の `remotePatterns` に追加が必要。`tests/test_image_remote_patterns.py`（backend CI）が `sites.yaml` との一致を強制する。列挙漏れは silent failure になる（PR #179 の教訓）。手動編集の代わりに `python3 scripts/sync_remote_patterns.py --fix` で不足ホストを自動追記できる（T104。50件上限を超える場合はワイルドカード方針の見直しが必要なため書き込まず停止する）
