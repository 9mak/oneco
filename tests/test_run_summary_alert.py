...
def test_diagnosis_count_alone_does_not_trigger_warning():
    """診断は別メッセージで通知済みのため、このサマリアラート自体は
    diagnosis_count だけでは鳴らない (他の異常シグナルが無ければ通知しない)。"""
    client = MagicMock()
    _send_run_summary_alert(
        notification_client=client,
        broken_tracker=_make_tracker(0),
...
def test_diagnosis_count_included_when_other_warning_present():
    """他の異常シグナル (件数ゼロ回帰等) がある場合、diagnosis_count が
    メッセージ・details に折り込まれる。"""
    from src.data_collector.infrastructure.site_baseline_tracker import ZeroCountRegression

    client = MagicMock()
    regressions = [
        ZeroCountRegression(
            site_name="サイトA", baseline_count=5, consecutive_zero_runs=2, last_nonzero_at=None
        )
    ]
    _send_run_summary_alert(
        notification_client=client,
        ...
        threshold=3,
        logger=_make_logger(),
        zero_count_regressions=regressions,
        diagnosis_count=2,
    )
    assert client.send_alert.called
    args, _ = client.send_alert.call_args
    details = client.send_alert.call_args[0][2]
    assert details["diagnosis_count"] == 2
    assert "構造診断 2 件" in args[1]
