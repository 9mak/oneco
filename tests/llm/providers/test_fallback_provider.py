...
class TestFallbackOnQuotaError:
    def test_falls_back_extract_animal_data_on_quota_error(self):
        ...
        fallback = _mock_provider(ExtractionResult(fields={"species": "猫"}, input_tokens=50, output_tokens=20))
    ...
    def test_falls_back_extract_detail_links_on_quota_error(self):
        primary = MagicMock()
    ...
    def test_falls_back_extract_multiple_animals_on_quota_error(self):
        ...
        fallback_result = MultiExtractionResult(
            animals=[{"species": "犬"}, {"species": "猫"}], input_tokens=200, output_tokens=100
        )
        fallback = _mock_provider(multi_result=fallback_result)

        provider = FallbackProvider(primary=primary, fallback=fallback)
        result = provider.extract_multiple_animals("PDF text", "https://example.com/list.pdf", "adoption")
...
class TestNoPrimaryWhenSuccess:
    def test_uses_primary_when_no_error(self):
        primary = _mock_provider(ExtractionResult(fields={"species": "犬"}, input_tokens=100, output_tokens=50))
