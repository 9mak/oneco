...
class TestWannyanFukuokaAdapterDetailExtraction:
    ...
    def test_extract_animal_details_returns_raw_data_dog(self, assert_raw_animal):
        ...
        # 「品種：雑種」は species ではなく breed。species は URL/サイト名から「犬」
        ...
        # sex "オス"→"男の子"、shelter_date "2026年4月21日"→date(2026, 4, 21)。
        ...
        assert animal_data.location == "東区香椎"
        assert animal_data.phone == "092-661-0500"
    ...
    def test_extract_animal_details_supports_two_column_table(self, assert_raw_animal):
        ...
        assert isinstance(raw, RawAnimalData)
        # 「品種：三毛猫」は species ではなく breed。species は URL/サイト名から「猫」。
        assert_raw_animal(
            raw,
            species="猫",
            breed="三毛猫",
