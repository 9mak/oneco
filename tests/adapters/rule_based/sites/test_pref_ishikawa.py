"""PrefIshikawaAdapter のテスト

石川県（保護犬・猫情報）(pref.ishikawa.lg.jp) 用 rule-based
adapter の動作検証。

- 対象ページは外部の動物愛護センター情報ページへのリンク + 各保健福祉
  センターの連絡先案内のみで、本文に動物個別データを持たないため
...
- 将来テンプレートに動物個別ブロックがインライン挿入された場合の
  テーブル抽出経路も担保する
- サイト名 "石川県（保護犬・猫情報）" の Registry 登録確認
"""
...
from data_collector.adapters.rule_based.sites.pref_ishikawa import (
    PrefIshikawaAdapter,
)
...
def _site(
    name: str = "石川県（保護犬・猫情報）",
    list_url: str = ("https://www.pref.ishikawa.lg.jp/yakuji/doubutsu/hogoinuneko.html"),
...
class TestPrefIshikawaAdapter:
    def test_fetch_animal_list_returns_empty_for_index_page(self, fixture_html):
        """外部リンク + 各センター連絡先のみの本文では空リストが返る

        fixture `pref_ishikawa_lg_jp.html` は実サイトと同じく本文中に
        動物個別ブロックを含まず、外部の動物愛護センター情報ページへの
        リンクと各保健福祉センターの連絡先のみが並ぶ典型的な
        「インデックス 0 件状態」のページ。基底実装は ParsingError を
        投げるが、本 adapter ではこれを正常な 0 件として扱う。
        """
        html = fixture_html("pref_ishikawa_lg_jp")
        adapter = PrefIshikawaAdapter(_site())
    ...
    def test_mojibake_is_repaired(self, fixture_html):
        ...
        adapter 側で逆変換することで、HTML キャッシュ内に「石川」等の
        日本語が読める状態となる。
        """
        html = fixture_html("pref_ishikawa_lg_jp")
        # 元 fixture には mojibake 状態の "ç³å·" が含まれ、
        # 直接の "石川" は含まれない (二重エンコードのため)
        assert "石川" not in html

        adapter = PrefIshikawaAdapter(_site())
        with patch.object(adapter, "_http_get", return_value=html):
            adapter.fetch_animal_list()

        # 復元後の HTML キャッシュには "石川" が含まれているはず
        assert adapter._html_cache is not None
        assert "石川" in adapter._html_cache

    def test_fetch_with_inline_table_returns_animal(self):
        """HTML 内に動物テーブルがインライン掲載されている場合は抽出される

        将来的にサイトが本文 (#tmp_contents) 内に動物テーブルを掲載した
        場合の挙動を担保する。
        """
        html = (
            "<html><head><title>犬猫の保護情報 | 石川県</title></head>"
            "<body><div id='tmp_contents'><table>"
            ...
            "<tr><th>収容場所</th><td>いしかわ動物愛護センター</td></tr>"
            "</table></div></body></html>"
        )
        adapter = PrefIshikawaAdapter(_site())
        with patch.object(adapter, "_http_get", return_value=html):
    ...
    def test_sidebar_and_index_links_are_not_extracted_as_rows(self, fixture_html):
        """サイドバー (`.col_rgt_navi`) や本文中の連絡先パラグラフは
        ROW として誤検出されない

        フィクスチャには本文に各保健福祉センターの連絡先 (`<p>`) や
        サイドナビゲーション (`.rnavi_func` 等) のリンクが並ぶが、
    ...
    def test_site_registered(self):
        """サイト名 "石川県（保護犬・猫情報）" が Registry に
        登録されている
        """
        name = "石川県（保護犬・猫情報）"
        if SiteAdapterRegistry.get(name) is None:
            SiteAdapterRegistry.register(name, PrefIshikawaAdapter)
        assert SiteAdapterRegistry.get(name) is PrefIshikawaAdapter

    def test_normalize_returns_animal_data(self):
        """RawAnimalData を normalize して AnimalData に変換できる"""
        html = (
            "<html><head><title>石川県</title></head>"
            "<body><div id='tmp_contents'><table>"
