"""CityNahaPdfAdapter のテスト

那覇市保護犬猫情報 PDF 系 rule-based adapter の動作を検証する。

- 一覧 HTML から PDF リンクを抽出 → 各 PDF を仮想 URL に展開
- PDF テキストから動物 dict を `_parse_pdf_text` で抽出
- _http_get / _download_pdf / _extract_pdf_text を mock し、合成 PDF テキストで検証
"""
...
from data_collector.adapters.rule_based.sites.city_naha_pdf import (
    CityNahaPdfAdapter,
)
...
_LIST_HTML = """
<html><head><title>那覇市 保護犬猫情報</title></head>
<body>
  <h1>保護犬猫情報</h1>
  <ul>
    <li><a href="/kurasitetuduki/animal/1002271/files/20260512.pdf">2026年5月12日収容分</a></li>
    <li><a href="/kurasitetuduki/animal/1002271/files/20260514.pdf">2026年5月14日収容分</a></li>
    <li><a href="/kurasitetuduki/animal/index.html">トップへ戻る</a></li>
  </ul>
</body></html>
"""

# 合成 PDF テキスト: 1 PDF に 2 頭分のデータ
_PDF_TEXT_TWO_ANIMALS = """那覇市 保護犬猫情報
...
def _site(name: str = "那覇市（保護犬猫情報）") -> SiteConfig:
    return SiteConfig(
        name=name,
        prefecture="沖縄県",
        prefecture_code="47",
        list_url=("https://www.city.naha.okinawa.jp/kurasitetuduki/animal/1002271/1002278.html"),
        category="sheltered",
    )
...
class TestParsePdfText:
    ...
    def test_parses_two_animals(self):
        adapter = CityNahaPdfAdapter(_site())
    ...
    def test_parses_one_animal_with_slash_date(self):
        ...
        assert "真和志" in records[0]["location"]
...
class TestFetchAndExtract:
    def test_fetch_animal_list_returns_virtual_urls(self):
        ...
        for url, cat in result:
            assert "#row=" in url
            assert url.startswith("https://www.city.naha.okinawa.jp/")
...
class TestRegistry:
    def test_naha_site_registered(self):
        """那覇市サイトが Registry に登録されている"""
        name = "那覇市（保護犬猫情報）"
        if SiteAdapterRegistry.get(name) is None:
            SiteAdapterRegistry.register(name, CityNahaPdfAdapter)
        assert SiteAdapterRegistry.get(name) is CityNahaPdfAdapter
