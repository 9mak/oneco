...
class TestCityKumamotoAdapterDetailExtraction:
    ...
    def test_extract_animal_details_returns_raw_data_dog(self, assert_raw_animal):
        ...
        # 「品種：雑種」は species ではなく breed。species はサイト名(迷子犬)から
        # 「犬」を推定する(以前は species="雑種"→normalizerで「その他」誤分類+breed欠落)。
        assert_raw_animal(
            raw,
            species="犬",
            breed="雑種",
        ...
        # shelter_date "2026年4月27日"→date(2026, 4, 27)。
        ...
        assert animal_data.shelter_date.isoformat() == "2026-04-27"
        assert animal_data.location == "北区植木町石川"
        assert animal_data.phone == "096-380-2153"
    ...
    def test_extract_animal_details_supports_two_column_table(self, assert_raw_animal):
        ...
        assert isinstance(raw, RawAnimalData)
        # 「品種：三毛猫」は species ではなく breed。species はサイト名(迷子猫)から「猫」。
        assert_raw_animal(
            raw,
            species="猫",
            breed="三毛猫",
        ...
        assert animal_data.shelter_date.isoformat() == "2026-04-21"
