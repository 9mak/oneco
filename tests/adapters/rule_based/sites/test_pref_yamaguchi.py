"""PrefYamaguchiAdapter のテスト

山口県動物愛護センター（収容動物）(pref.yamaguchi.lg.jp) 用 rule-based
adapter の動作検証。

- 対象ページは県内 8 健康福祉センターの収容情報ページへのリンク集インデックスで、
...
- 将来テンプレートに動物個別ブロックがインライン挿入された場合のテーブル
  抽出経路も担保する
- サイト名 "山口県動物愛護センター（収容動物）" の Registry 登録確認
"""
...
from data_collector.adapters.rule_based.sites.pref_yamaguchi import (
    PrefYamaguchiAdapter,
)
...
class TestPrefYamaguchiAdapter:
    def test_fetch_animal_list_returns_empty_for_index_page(
        ...
        動物個別ブロックを含まず、8 健康福祉センター等への内部リンクと
        注意事項のみが並ぶ典型的な「インデックス 0 件状態」のページ。
        基底実装は ParsingError を投げるが、本 adapter ではこれを
        正常な 0 件として扱う。
        """
        html = fixture_html("pref_yamaguchi_lg_jp")
        adapter = PrefYamaguchiAdapter(_site())
    ...
    def test_mojibake_is_repaired(self, fixture_html):
        ...
        adapter 側で逆変換することで、HTML キャッシュ内に「山口」等の
        日本語が読める状態となる。
        """
        html = fixture_html("pref_yamaguchi_lg_jp")
        # 元 fixture には mojibake 状態の "å±±å£" が含まれ、
        # 直接の "山口" は含まれない (二重エンコードのため)
        assert "山口" not in html

        adapter = PrefYamaguchiAdapter(_site())
        with patch.object(adapter, "_http_get", return_value=html):
            adapter.fetch_animal_list()

        # 復元後の HTML キャッシュには "山口" が含まれているはず
        assert adapter._html_cache is not None
        assert "山口" in adapter._html_cache

    def test_fetch_with_inline_table_returns_animal(self):
        """HTML 内に動物テーブルがインライン掲載されている場合は抽出される

        将来的にサイトが本文 (#main_body) 内に動物テーブルを掲載した場合の
        挙動を担保する。
        """
        html = (
            "<html><head><title>収容犬・猫情報 | 山口県</title></head>"
            "<body><div id='main_body'><table>"
            ...
            "<tr><th>収容場所</th><td>山口健康福祉センター</td></tr>"
            "</table></div></body></html>"
        )
        adapter = PrefYamaguchiAdapter(_site())
        with patch.object(adapter, "_http_get", return_value=html):
    ...
    def test_sidebar_and_index_links_are_not_extracted_as_rows(
        ...
        フィクスチャには `#main_body div.info_list` 配下に各健康福祉センター
    ...
    def test_site_registered(self):
        """サイト名 "山口県動物愛護センター（収容動物）" が Registry に
        登録されている
        """
        name = "山口県動物愛護センター（収容動物）"
        if SiteAdapterRegistry.get(name) is None:
            SiteAdapterRegistry.register(name, PrefYamaguchiAdapter)
        assert SiteAdapterRegistry.get(name) is PrefYamaguchiAdapter

    def test_normalize_returns_animal_data(self):
        """RawAnimalData を normalize して AnimalData に変換できる"""
        html = (
            "<html><head><title>山口県</title></head>"
            "<body><div id='main_body'><table>"
