...
# ソースサイトで last_seen_at がこの日数以上更新されていない動物は「消えた」と
# みなして公開面で非表示にする。収集は日次のため、数日のサイト一時障害では
# 消えないよう保守的な既定値 (7日) とし、ANIMAL_STALE_HIDE_DAYS で調整可能。
_DEFAULT_STALE_HIDE_DAYS = 7


def _stale_cutoff() -> datetime:
    """これより前にしか確認されていない動物は非表示にする閾値時刻を返す。"""
    try:
        days = int(os.getenv("ANIMAL_STALE_HIDE_DAYS", str(_DEFAULT_STALE_HIDE_DAYS)))
    except ValueError:
        days = _DEFAULT_STALE_HIDE_DAYS
    return datetime.now(UTC) - timedelta(days=max(days, 0))


def _not_disappeared(cutoff: datetime):
    """公開表示対象の鮮度条件。last_seen_at が NULL (未記録) の行は隠さない。"""
    return or_(Animal.last_seen_at.is_(None), Animal.last_seen_at >= cutoff)
...
class AnimalRepository:
    ...
    async def save_animal(self, animal_data: AnimalData) -> AnimalData:
        ...
        # 収集で確認できた = 今ソースに掲載されている、の証跡として更新する。
        # これが古いままの動物は「ソースから消えた」とみなし公開面で非表示化する。
        orm_animal.last_seen_at = datetime.now(UTC)
    ...
    async def list_animals(
        ...
        if not include_hidden:
            # ソースから消えた (last_seen_at が古い) 動物を公開面から除外する
            filters.append(_not_disappeared(_stale_cutoff()))
