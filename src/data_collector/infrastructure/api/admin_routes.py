...
from src.data_collector.adapters.rule_based.broken_tracker import BrokenSitesTracker
...
# broken_sites.yaml は __main__.py と同じ規約 (BROKEN_SITES_PATH env で差し替え可)
_BROKEN_SITES_PATH = Path(os.environ.get("BROKEN_SITES_PATH", "data/broken_sites.yaml"))
# consec >= この値で auto-skip 対象 (__main__.py の BROKEN_SKIP_THRESHOLD と一致させる)
_BROKEN_SKIP_THRESHOLD = 3
...
def _load_broken_tracker() -> BrokenSitesTracker | None:
    """broken_sites.yaml が存在すれば BrokenSitesTracker を返す。

    ファイル未生成時は None を返し、呼び出し側で「全サイト healthy」とみなす。
    """
    try:
        return BrokenSitesTracker(_BROKEN_SITES_PATH)
    except Exception as e:
        logger.warning(f"broken_sites.yaml load 失敗: {e}")
        return None


def _site_health_for(tracker: BrokenSitesTracker | None, site_name: str) -> dict:
    """サイト 1 つの健全性ステータスを返す。

    status:
      - ok       : 失敗記録なし
      - warning  : 1-2 回連続失敗 (auto-skip 閾値未満)
      - failing  : threshold 以上で auto-skip 状態
    """
    if tracker is None:
        return {
            "status": "ok",
    ...
    consec = tracker.consecutive_failures(site_name)
    if consec == 0:
        status = "ok"
    elif consec < _BROKEN_SKIP_THRESHOLD:
        status = "warning"
    else:
        status = "failing"
    entry = tracker._state.get(site_name, {})
    return {
        "status": status,
...
async def list_admin_sites(
    ...
    """
    sites.yaml に登録された全サイトの一覧と、各サイトの DB 上の収集状況、
    および broken_sites.yaml に基づく健全性ステータスを返す。

    collection-ops-dashboard spec Req2/Req6 の実装。
    """
    try:
        config = SiteConfigLoader.load(_SITES_YAML_PATH)
    except Exception as e:
    ...
    tracker = _load_broken_tracker()
    summary = {"ok": 0, "warning": 0, "failing": 0}
    ...
    for site in config.sites:
        ...
        health = _site_health_for(tracker, site.name)
