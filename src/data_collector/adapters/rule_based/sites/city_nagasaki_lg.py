...
確認だけ行って 0 件を返す。名古屋市の譲渡犬・譲渡猫 (W001/T017) と同じ
「取得不能」の扱い。

サイト定義 (`sites.yaml`) を残しているのは掲載再開に気づく余地を残すため
だが、**現時点で自動アラートは無い** (異常検知は W002/T101 が未実装)。
気づく手段は `zero_count_audit` の手動実行と収集ログの目視に限られる。
"""
...
class CityNagasakiLgAdapter(SinglePageTableAdapter):
    ...
    掲載が始まったときに気づく余地を残すため (自動アラートは未実装)。
    """
