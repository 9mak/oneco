"""山形県（飼い主探し掲示板・ハブ）rule-based adapter

対象ドメイン: https://www.pref.yamagata.jp/020071/kenfuku/doubutsuaigo/aigo/kainushisagashi/

特徴:
- 対象ページ (`keijiban.html`) は山形県健康福祉部の「犬・猫の飼い主探し掲示板」
  インデックスページであり、ここ自体には個別の動物情報は掲載されない。代わりに
  県内 4 保健所 (村山 / 最上 / 置賜 / 庄内) の飼い主探し掲示板ページと、
  各保健所の保護・収容動物情報ページへのリンク一覧が並ぶ典型的な「ハブ」ページ構成。
- ページ HTML は本文中に動物個別ブロックを持たないため、`fetch_animal_list`
  は通常 0 件 (空リスト) を返す。これは `pref_yamaguchi` / `pref_kanagawa`
...
- 山形県の HTML レスポンスは fixture 上で UTF-8 バイト列を Latin-1 として
  誤解釈し再 UTF-8 化された二重エンコーディング (mojibake) になっている
  ことがあり、`_load_rows` で「山形」が含まれていない場合に限って
  逆変換 (latin-1 → utf-8) を試みる。実運用 (`requests`) では正しい UTF-8
  で受け取るため、この処理は冪等に no-op となる。
- 動物種別 (犬/猫/その他) はサイト名 (「飼い主探し掲示板」) からは特定できないため、
...
- 山形県のテンプレートは本文を `#tmp_main` 配下にラップするため、サイドバー
  (`#tmp_lnavi`) の保健所リンク一覧を ROW として誤検出しないよう、
  `ROW_SELECTOR` は本文 (`#tmp_main` 配下の動物個別ブロックや table) に限定する。
"""
...
class PrefYamagataAdapter(SinglePageTableAdapter):
    """山形県（飼い主探し掲示板・ハブ）用 rule-based adapter

    インデックスページが「県内 4 保健所の飼い主探し掲示板リンク集」と
    ...
    # 動物個別ブロックの典型候補:
    # - 県の WordPress 系 / 自治体テンプレートで使われる `div.p-card-detail` 等
    # - 詳細属性テーブル
    # 本文 (#tmp_main) 配下に限定し、サイドバー (#tmp_lnavi) や
    # フッター・パンくずを誤検出しないようにする。
    ROW_SELECTOR: ClassVar[str] = (
        "#tmp_main div.p-card-detail, "
        "#tmp_main div.p-animal-detail, "
        "#tmp_main div.animal-card, "
        "#tmp_main #tmp_contents table"
    )
    ...
    def fetch_animal_list(self) -> list[tuple[str, str]]:
        ...
        山形県の本ページは通常「4 保健所のリンク集 + 注意事項」のみで
...
# ─────────────────── サイト登録 ───────────────────
# sites.yaml の `name: 山形県（飼い主探し掲示板・ハブ）` と完全一致させる。
_SITE_NAME = "山形県（飼い主探し掲示板・ハブ）"
if SiteAdapterRegistry.get(_SITE_NAME) is None:
    SiteAdapterRegistry.register(_SITE_NAME, PrefYamagataAdapter)
