"""和歌山県（保護犬猫情報）rule-based adapter

対象ドメイン: https://www.pref.wakayama.lg.jp/

特徴:
- 対象ページ (`/prefg/031601/d00156970.html`) は和歌山県動物愛護センター直下の
  「収容犬・猫の情報」インデックスページであり、ここ自体には個別の動物情報は
  掲載されない。代わりに県内 9 施設 (橋本/岩出/海南/湯浅/御坊/田辺/新宮/
  新宮串本支所/和歌山市動物愛護管理センター) の収容情報ページへのリンク一覧
  と、注意事項 (返還には飼い主証明が必要、手数料がかかる等) が並ぶ。
- ページ HTML は本文中に動物個別ブロックを持たないため、`fetch_animal_list`
  は通常 0 件 (空リスト) を返す。これは `pref_kanagawa` adapter と同様の
  「PDF/外部サイト配布で本文 0 件状態が常態」パターンと整合する。
- 将来的に同じテンプレート上へ動物個別ブロック (テーブル / `div.p-card-detail`
  系) がインライン掲載されるよう変更された場合に備え、典型的な動物カード /
  テーブル要素を `ROW_SELECTOR` で拾えるようにしておく。検出された場合は
  ラベル → フィールドのマップに従って属性を取り出す。
- 和歌山県の HTML レスポンスは fixture 上で UTF-8 バイト列を Latin-1 として
  誤解釈し再 UTF-8 化された二重エンコーディング (mojibake) になっている
  ことがあり、`_load_rows` で「和歌山」が含まれていない場合に限って
  逆変換 (latin-1 → utf-8) を試みる。実運用 (`requests`) では正しい UTF-8
  で受け取るため、この処理は冪等に no-op となる。
- 動物種別 (犬/猫/その他) はサイト名 (例: 「和歌山県（保護犬猫情報）」)
  からは犬猫いずれも特定できないため、HTML 中に種別が明示されている場合は
  それを優先し、見つからなければ空文字とする。
...
class PrefWakayamaAdapter(SinglePageTableAdapter):
    """和歌山県（保護犬猫情報）用 rule-based adapter

    インデックスページが「県内 9 保健所等の収容情報リンク集」となっており、
    本文には動物個別データが無い (= 0 件) のが通常運用。
    ...
    # - 詳細属性テーブル
    # 本文の枠 (#content / div.article) 配下に限定し、
    # サイドバー (#sidebar) の組織リンク等を誤検出しないようにする。
    ROW_SELECTOR: ClassVar[str] = (
        "#content div.p-card-detail, #content div.p-animal-detail, #content div.article table"
    )
    ...
    def _load_rows(self) -> list[Tag]:
        ...
        # mojibake 検出: ページに「和歌山」が含まれない場合は
    ...
    def fetch_animal_list(self) -> list[tuple[str, str]]:
        ...
        和歌山県の本ページは通常「9 保健所のリンク集 + 注意事項」のみで
    ...
    def extract_animal_details(
        ...
        両方に対応する。ラベル → フィールドのマップは pref_kanagawa と同等。
        """
        rows = self._load_rows()
        ...
        # 和歌山県動物愛護センター固有のテンプレートが入手できていないため、
...
# ─────────────────── サイト登録 ───────────────────
# sites.yaml の `name: 和歌山県（保護犬猫情報）` と完全一致させる。
# 和歌山市動物愛護管理センターは別サイト名 (= 別 adapter 候補) のため含まない。
_SITE_NAME = "和歌山県（保護犬猫情報）"
if SiteAdapterRegistry.get(_SITE_NAME) is None:
    SiteAdapterRegistry.register(_SITE_NAME, PrefWakayamaAdapter)
