...
class LlmAdapter(MunicipalityAdapter):
    ...
    def fetch_animal_list(self) -> list[tuple[str, str]]:
        ...
        current_url: str | None = self.site_config.list_url
        pages_visited = 0

        while current_url:
            if self.site_config.max_pages and pages_visited >= self.site_config.max_pages:
                logger.info(
        ...
        logger.info(f"[{self.site_config.name}] {len(all_urls)} 件の詳細URLを収集")
        return all_urls
    ...
    def _expand_multi_animal_pdf(self, pdf_url: str, category: str) -> list[tuple[str, str]]:
        """
    ...
    def _get_from_multi_cache(self, virtual_url: str, category: str) -> RawAnimalData:
        """
    ...
    def _fetch_page(self, url: str) -> str:
        ...
        if self.site_config.requires_js:
            fetcher: PageFetcher = PlaywrightFetcher(wait_selector=self.site_config.wait_selector)
        else:
    ...
    def _extract_detail_urls(self, html: str, base_url: str) -> list[str]:
        ...
        if self.site_config.pdf_link_pattern:
            # CSSセレクターでPDFリンクを抽出
            return self._extract_urls_by_selector(html, base_url, self.site_config.pdf_link_pattern)
    ...
    def _extract_urls_by_selector(self, html: str, base_url: str, selector: str) -> list[str]:
        """CSSセレクターでリンクを抽出"""
    ...
    def _find_next_page(self, html: str, current_url: str) -> str | None:
        ...
        for pattern in next_patterns:
            link = soup.find("a", string=lambda s, p=pattern: s and p in s)
