...
from .deceased_notice import deceased_notice
from .models import AnimalData, AnimalStatus, RawAnimalData
...
class DataNormalizer:
    ...
    def normalize(raw_data: RawAnimalData) -> AnimalData:
        ...
        # 備考に「死亡確認」等がある個体は公開から外す (T424)。status を立てるだけで、
        # 公開クエリ側 (repository) が deceased を除外する。死亡記載が無いときは None
        # のままにして、管理 API で付けた adopted/returned を毎日の収集で戻さない。
        description = DataNormalizer._normalize_description(raw_data.description)
        status: AnimalStatus | None = None
        if matched := deceased_notice(description):
            status = AnimalStatus.DECEASED
            logger.warning(
                "備考に死亡の記載があるため公開から外します",
                extra={
